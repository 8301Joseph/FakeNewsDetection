...
1. **Data Preprocessing**: `lemmatization.py` → `lemmatized.csv`
2. **Feature Extraction** (choose one or more):
   - **TF-IDF**: `TF-IDF.py` → TF-IDF matrices and analysis
   - **Doc2Vec**: `doc2vec.py` → `doc2vec_output.csv`
   - **Word2Vec**: `word2vec.py` → `word2vec_output.csv`
   - **Sentence2Vec**: `sentence2vec.py` → `sentence2vec.csv`
3. **Train/Test Split**: `traintest.py` → `train_set.csv`, `test_set.csv`
